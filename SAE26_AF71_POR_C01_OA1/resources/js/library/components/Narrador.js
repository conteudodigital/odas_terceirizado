...
export class Narrador {
   ...
   * @param {Object} colors - estilo do botão "PULAR" (se usado)
   * @param {Function} onFinish - callback ao finalizar (ex.: métricas/cleanup)
  ...
  constructor(
    ...
    this.hud = new TextFrameHUD(scene, 0, 0, this.textFrameKey, 0, {
    ...
    this.hud.onAvancar(() => {
      // 1) tenta rodar o onFinish sem travar a navegação
      try {
    ...
    if (this.permitirPular) {
      ...
      if (!pularColors) {
        try {
          ...
          pularColors = ColorManager.getColors(marca, ColorManager.BLUE);
        } catch (e) {
          pularColors = {
            background: 0xffd60a,
            border: 0x000000,
