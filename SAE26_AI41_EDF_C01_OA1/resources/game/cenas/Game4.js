...
export class Game4 extends BaseCena {
  constructor(controladorDeCenas) {
    ...
    this.LAYOUT = {
      ...
      LINE_AREA: { key: "linha_atv2", x: 1460, y: 645, scale: 1.0 },
      ...
      ATVBALLS: [
        { key: "bolinha_laranja", x: 1715, y: 645, scale: 1.0 },
        { key: "bolinha_rosa", x: 1453, y: 645, scale: 1.0 },
        { key: "bolinha_verde", x: 1190, y: 645, scale: 1.0 },
      ],
  ...
  create() {
    ...
    const lineCfg = this.LAYOUT.LINE_AREA;
    this.lineArea = this.add
      .image(lineCfg.x, lineCfg.y, lineCfg.key)
      .setOrigin(0.5)
      .setScale(lineCfg.scale ?? 1)
      .setDepth(this.DEPTH.LINE);
  ...
  _checkLineDepartures() {
    const yLine = this.LAYOUT.LINE_REF.y;
    const tol = this.LAYOUT.LINE_REF.tol ?? 16;

    const bandTop = yLine - tol;
    const bandBottom = yLine + tol;

    const rectW = this.lineArea ? this.lineArea.displayWidth : 800;
    const rectX0 =
      (this.lineArea ? this.lineArea.x : this.LAYOUT.LINE_AREA.x || 1460) -
      rectW / 2;
    const rectX1 = rectX0 + rectW;
    ...
    this.atvBalls.forEach((ball) => {
      ...
      const r =
        (ball.body && ball.body.height
          ? ball.body.height
          : ball.displayHeight) * 0.5;

      const top = ball.y - r;
      const bottom = ball.y + r;
      const left = ball.x - r;
      const right = ball.x + r;

      const overlapsY = !(bottom < bandTop || top > bandBottom);
      const overlapsX = !(right < rectX0 || left > rectX1);
      const intersectsRect = overlapsX && overlapsY;

      const fullyOutside = !intersectsRect;
  ...
  _showFeedbackOK() {
    ...
    if (this.nextSceneTimer) this.nextSceneTimer.remove(false);
    this.nextSceneTimer = this.time.delayedCall(3000, () =>
      this._goToNextScene()
    );
  }
