...
export class Enunciado {
  constructor(scene) {
    this.scene = scene;
    /*console.log('Scene no Enunciado:', scene);
        console.log('Game no Enunciado:', scene.game);*/

    if (!SoundManager.game) {
      //console.log('Inicializando SoundManager no Enunciado');
  ...
  create() {
    ...
    this.btNarracao = this.scene.add
      .image(
    ...
    const audioTexto = [
      {
        audio: "NA002",
        texto:
          "Ajudando BErnardo. PARA CONHECER UM POUCO MAIS SOBRE A PRÓPRIA FAMÍLIA, BERNARDO DECIDIU CRIAR UMA ÁRVORE GENEALÓGICA. AJUDE BERNARDO A CRIAR A ÁRVORE DA SUA FAMÍLIA!",
      },
    ];

    this.btNarracao.on("pointerdown", () => {
      if (this.audio) {
        // Verifica se existe um áudio definido
    ...
    // Botão Vamos Lá
    const btVamosLa = this.scene.add
      .image(
        this.imageBox.x,
  ...
  updateContent(options = {}) {
    ...
    if (text && this.mainText) {
      ...
      this.mainText = this.scene.add
        .rexTagText(this.textBox.x, this.textBox.y, text, mainTextStyle)
        .setOrigin(0.5, 0.54); // Centralizar horizontalmente e verticalmente
      this.container.add(this.mainText);

      if (icon) {
        if (icon.nome != "" || icon.nome) {
          const icone = this.scene.add
            .image(icon.posicao.x, icon.posicao.y, icon.nome)
            .setOrigin(0, 0);
